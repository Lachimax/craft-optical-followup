...
data_dir=$(jq -r .data_dir "param/epochs_sdss/${param_file}.json")
back_size=$(jq -r .sextractor_field_back_size "param/epochs_sdss/${param_file}.json")
threshold=$(jq -r .threshold "param/epochs_sdss/${param_file}.json")
...
if cd "${sextractor_destination_path}" ; then
    ...
    for image in $(ls *cutout.fits) ; do
        ...
        python3 "${proj_dir}/pipeline_fors2/9-psf.py" --directory "${data_dir}" --psfex_file "${sextractor_destination_path}${image_f}_psfex.psf" --image_file "${sextractor_destination_path}${image}" --prefix "${image_f}"
