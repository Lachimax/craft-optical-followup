  ...
  for image in *astrometry.fits; do
    ...
    if [[ ${image_f} != "${df}" ]]; then
      if ${do_dual_mode}; then
        sextractor "${df}_astrometry.fits,${image}" -c psf-fit.sex -CATALOG_NAME "${image_f}_dual-mode.cat" -PSF_NAME "${image_f}_psfex.psf" -SEEING_FWHM "${fwhm}" -BACK_SIZE "${back_size}" -DETECT_THRESH "${threshold}" -ANALYSIS_THRESH "${threshold}"
        cd "${proj_dir}" || exit
...
python3 "scripts/zeropoint.py" --op "${data_title}" -write --instrument XSHOOTER -show --mag_tolerance 0.5 #-not_stars_only