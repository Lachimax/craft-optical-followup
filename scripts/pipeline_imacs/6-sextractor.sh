...
if cd "${sextractor_destination_path}"; then
  ...
  for image in *coadded.fits; do
    ...
    python3 "${proj_dir}/scripts/pipeline_imacs/6-psf.py" --directory "${data_dir}" --psfex_file "${sextractor_destination_path}${image_f}_psfex.psf" --image_file "${sextractor_destination_path}${image}" --prefix "${image_f}"
    cd "${sextractor_destination_path}" || exit
    ...
    if [[ ${image_f} != "${df}" ]]; then
      if ${do_dual_mode}; then
        ...
        cd "${proj_dir}" || exit
        python3 scripts/add_path.py --op "${data_title}" --key "${image_f}_cat_path" --path "${sextractor_destination_path}${image_f}_psf-fit.cat" --instrument IMACS
      fi
    else
      cd "${proj_dir}" || exit
      python3 scripts/add_path.py --op "${data_title}" --key "${image_f}_cat_path" --path "${sextractor_destination_path}${image_f}_psf-fit.cat" --instrument IMACS
