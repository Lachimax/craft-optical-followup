...
do_dual_mode=$(jq -r .do_dual_mode "param/epochs_imacs/${param_file}.json")
back_size=$(jq -r .sextractor_field_back_size "param/epochs_imacs/${param_file}.json")
deepest_filter=$(jq -r .deepest_filter "param/epochs_imacs/${param_file}.json")
threshold=$(jq -r .threshold "param/epochs_imacs/${param_file}.json")
df=${deepest_filter::1}

sextractor_destination_path=${data_dir}${destination}

mkdir "${sextractor_destination_path}"
cp "${data_dir}${origin}"*coadded* "${sextractor_destination_path}"
if cd "${sextractor_destination_path}"; then
  ...
  for image in *coadded.fits; do
    ...
    python3 "${proj_dir}/pipeline_imacs/6-psf.py" --directory "${data_dir}" --psfex_file "${sextractor_destination_path}${image_f}_psfex.psf" --image_file "${sextractor_destination_path}${image}" --prefix "${image_f}"
    cd "${sextractor_destination_path}" || exit
    ...
    if [[ ${image_f} != "${df}" ]]; then
      if ${do_dual_mode}; then
        ...
        cd "${proj_dir}" || exit
        python3 /add_path.py --op "${param_file}" --key "${image_f}_cat_path" --path "${sextractor_destination_path}${image_f}_dual-mode.cat" --instrument IMACS
      else
        cd "${proj_dir}" || exit
        python3 /add_path.py --op "${data_title}" --key "${image_f}_cat_path" --path "${sextractor_destination_path}${image_f}_psf-fit.cat" --instrument IMACS
      fi
    else
      cd "${proj_dir}" || exit
      python3 /add_path.py --op "${data_title}" --key "${image_f}_cat_path" --path "${sextractor_destination_path}${image_f}_psf-fit.cat" --instrument IMACS
